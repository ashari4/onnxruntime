...
class _TensorStub(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, _TensorStub):
            ...
            return False
        elif self.name != self.name != other.name:
            return False
        elif self.dtype != self.dtype != other.dtype:
            return False
        elif self.shape != self.shape != other.shape:
            return False
        elif self.shape_dims != self.shape_dims != other.shape_dims:
            return False
        return True
...
def _extract_schema(data):
    ...
    # Depth first traversal to iterate over the data to replace every tensor with a stub
        ...
        return _TensorStub(None, str(data.dtype), None, len(data.size()))
...
def parse_inputs_for_onnx_export(all_input_parameters, onnx_graph, *inputs, **kwargs):
    ...
    schema = _extract_schema({'args' : copy.copy(inputs),
