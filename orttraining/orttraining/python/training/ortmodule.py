...
class ORTModule(torch.nn.Module):
    ...
    def __init__(self, module):
        ...
        def _forward(self, *inputs, **kwargs):
            ...
            # Exporting module to ONNX for the first time or when input changes
            schema = _io._extract_schema({'args' : copy.copy(inputs),
                                          'kwargs' : copy.copy(kwargs)})
            build_gradient_graph = False
            if not self._onnx_training or self._input_info.schema != schema:
            ...
            input_info, input_info.schema = _io.parse_inputs_for_onnx_export(self._original_module_parameters,
            ...
            return _io.populate_user_output_from_schema_and_outputs(
                self._original_module_output_schema,
                self._onnx_graphs_info.user_output_names,
                _ORTModuleFunction.apply(
                    *_io._convert_input_to_list(self._flattened_output_module.named_parameters(),
                                                self._onnx_graphs_info.user_input_names,
        ...
        self._flattened_output_module = \
            _io.get_flattened_output_module(
                self._original_module)
    ...
    def _build_training_graph(self):
        if self._use_static_shape:
            self._module_gradient_graph_builder.build(self._input_info.shape)
        else:
