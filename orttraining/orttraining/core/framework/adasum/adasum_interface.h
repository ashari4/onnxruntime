...
static inline bool IsPowerOfTwo(unsigned x) { return (x != 0) && ((x & (x - 1)) == 0); }

// Interface for Adasum algorithm
template <typename Communicator_type>
class AdasumInterface {
 public:
  Status DispatchFusedAllreduce(void* grad_buffer, void* recv_buffer, std::vector<int>& tensor_counts, int start_level,
                                Communicator_type communicator, int tag, const Communicator_type* reduction_comms,
                                MLDataType data_type) {
    if (data_type == DataTypeImpl::GetType<MLFloat16>()) {
      FusedAllreduce((uint16_t*)grad_buffer, (uint16_t*)recv_buffer, data_type, tensor_counts, start_level,
                     communicator, tag, reduction_comms);
    } else if (data_type == DataTypeImpl::GetType<float>()) {
      FusedAllreduce((float*)grad_buffer, (float*)recv_buffer, data_type, tensor_counts, start_level, communicator, tag,
                     reduction_comms);
    } else if (data_type == DataTypeImpl::GetType<double>()) {
      FusedAllreduce((double*)grad_buffer, (double*)recv_buffer, data_type, tensor_counts, start_level, communicator,
                     tag, reduction_comms);
    } else {
 ...
 protected:
  ...
  virtual void PointToPointSendRecv(void* input_data_buffer, int64_t input_buffer_bytes, void* output_data_buffer,
                                    int64_t output_buffer_bytes, MLDataType data_type, int dst_src_rank, int tag,
  ...
  virtual void DispatchComputeDotAndNormSqrds(const void* __restrict__ a, const void* __restrict__ b,
                                              MLDataType data_type, int count, double& dotProduct, double& anormsq,
                                              double& bnormsq) {
    if (data_type == DataTypeImpl::GetType<MLFloat16>()) {
  ...
  virtual void DispatchScaledAdd(MLDataType data_type, int count, double acoeff, void* __restrict__ a, double bcoeff,
                                 void* __restrict__ b) {
    if (data_type == DataTypeImpl::GetType<MLFloat16>()) {
 ...
 private:
  ...
  template <typename T>
  void FusedAllreduce(T* grad_buffer, T* recv_buffer, MLDataType data_type, std::vector<int>& tensor_counts,
                      int start_level, Communicator_type communicator, int tag,
    ...
    if (IsPowerOfTwo(size) == false) {
      ORT_THROW("Adasum doesn't currently support reduction among non-power-of-2 number of ranks.");
    }
    ...
    for (size_t i = 0; i < tensor_counts.size(); i++) total_counts_sum += tensor_counts[i];
    ...
    for (level = 1, comm_index = 0; level < size; level = (level << 1), comm_index++) {
      ...
      this->PointToPointSendRecv((uint8_t*)(&grad_buffer[sendOffset]), nghrCount * per_element_size,
                                 (uint8_t*)(&recv_buffer[recvOffset]), myCount * per_element_size, data_type,
                                 neighbor_rank, tag, communicator);
      if ((rank & level) != 0) {
        grad_buffer = &grad_buffer[nghrCount];
        recv_buffer = &recv_buffer[nghrCount];
      }
      FusedPairwiseReduceWithComm((uint8_t*)grad_buffer, (uint8_t*)recv_buffer, data_type, tensor_counts,
                                  reduction_comms[comm_index], (rank & level) == 0, normAndDots);
    }
    ...
    for (level = (size >> 1); level > 0; level = (level >> 1)) {
      ...
      this->PointToPointSendRecv(grad_buffer, myCount * per_element_size, recv_buffer, nghrCount * per_element_size,
  ...
  void FusedPairwiseReduceWithComm(uint8_t* a, uint8_t* b, MLDataType data_type, std::vector<int>& tensor_counts,
    ...
    for (size_t i = 0; i < tensor_counts.size(); i++) {
      ...
      DispatchComputeDotAndNormSqrds(&a[bytesSoFar], &b[bytesSoFar], data_type, tensor_counts[i], dotProduct, anormsq,
                                     bnormsq);
    ...
    SumAllreduceWithComm((void*)normAndDots.data(), 3 * tensor_counts.size(), DataTypeImpl::GetType<double>(), comm);
  ...
  template <typename T>
  void ComputeDotAndNormSqrds(const T* __restrict__ a, const T* __restrict__ b, int count, double& dotProduct,
